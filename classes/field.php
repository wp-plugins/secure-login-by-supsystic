...
class fieldSln {
	 ...
	 */
	public $adapt = array('htmlSln' => '', 'dbFrom' => '', 'dbTo' => '');
	/**
	...
	public function setValue($value, $fromDB = false) {
		if(isset($this->adapt['dbFrom']) && $this->adapt['dbFrom'] && $fromDB)
			$value = fieldAdapterSln::_($value, $this->adapt['dbFrom'], fieldAdapterSln::DB);
		$this->value = $value;
	}
	...
	public function drawHtml($tag, $id) {
		if(method_exists('htmlSln', $this->html)) {
			$method = $this->html;
			...
			if($this->adapt['html']) {
				fieldAdapterSln::_($this, $this->adapt['html'], fieldAdapterSln::HTML);
			}
			$params = $this->processParams($tag, $id);
			if ($params != '')
				return $params;
			if ($this->name == 'default_value') {
				$optionsFromDb = frameSln::_()->getModule('optionsSln')->getHelper()->getOptions($id);
				if (!empty($optionsFromDb)) {
					$options = array(0 => __('Select', SLN_LANG_CODE));
					foreach($optionsFromDb as $k => $v)
						$options[$k] = $v;
					$method = 'selectbox';
					$this->htmlParams['optionsSln'] = $options;
				}
			}
			$htmlContent = htmlSln::$method($this->name, $this->htmlParams);
			if(!empty($this->errorEl)) {
				if(is_bool($this->errorEl))
					$errorEl = '<div class="toeErrorForField toe_'. htmlSln::nameToClassId($this->name). '"></div>';
	...
	public function displayValue() {
		...
		switch($this->html) {
			case 'countryList':
				$value = fieldAdapterSln::displayCountry($this->value);
				break;
			case 'statesList':
				$value = fieldAdapterSln::displayState($this->value);
				break;
			case 'checkboxlist':
				$options = $this->getHtmlParam('optionsSln');
				$value = array();
	...
	public function viewField($name, $fieldValue = '') {
		$method = $this->html;
		$options = frameSln::_()->getModule('optionsSln')->getHelper()->getOptions($this->id);
		$attrs = '';
		...
		$params = array('optionsSln'=>$options, 'attrs' => $attrs, 'value' => $fieldValue, 'checked' => $checked);
		$output = '';
		if(method_exists('htmlSln', $method)) {
			$output .= htmlSln::$method($name, $params);
			$output .= htmlSln::hidden('extra_field['.$this->name.']',array('value'=>$this->id));
		}
		return $output;
	}
	...
	public function processParams($tag, $id){
		...
		if ($this->name == "params") {
			...
			switch ($tag) {
				case 5: 
					$add_option = __('Add Checkbox', SLN_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 9: 
					$add_option = __('Add Item', SLN_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 12:
				...
				case 10:
					$options_tag = '';
					$add_option = __('Add Radio Button', SLN_LANG_CODE);
					$image_tag = ' style="display:none"';
				break;
				case 8:
					$image_tag = '';
			...
			if ($tag > 0 || $id == 0) {
				$output .= '<div class="options options_tag"'.$options_tag.'>';
					...
					$output .= fieldAdapterSln::_($id,'getExtraFieldOptions',fieldAdapterSln::STR);
				$output .= '</div>';

				$output .= '<div class="options image_tag"'.$image_tag.'>'.__('Dimensions', SLN_LANG_CODE).':<br />';
					$params->width?$width = $params->width:'';
					$params->height?$height = $params->height:'';
					$output .= __('width', SLN_LANG_CODE).':<br />';
					$output .= htmlSln::text('params[width]',array('value'=>$width)).'<br />';
					$output .= __('height', SLN_LANG_CODE).':<br />';
					$output .= htmlSln::text('params[height]',array('value'=>$height)).'<br />';
				$output .= '</div>';
			}
			if($this->adapt['htmlParams']) {
				$output .= fieldAdapterSln::_($this, $this->adapt['htmlParams'], fieldAdapterSln::STR);
			} else {
				$output .= '<a href="javascript:void(0);" class="set_properties">'.__('Click to set field "id" and "class"', SLN_LANG_CODE).'</a>';
				$output .= '<div class="attributes" style="display:none;">'.__('Attributes', SLN_LANG_CODE).':<br />';
				$output .= fieldAdapterSln::_($params,'getFieldAttributes',  fieldAdapterSln::STR);
				$output .= '</div>';
			}
			return $output;
		}
	...
	function checkVarFromParam($param, $element) {
		return utilsSln::xmlAttrToStr($param, $element);
	  ...
	  // load xml structure of parameters
	   ...
	   foreach ($config->params->param as $param) {
		 // read the variables
		  ...
		  $helper = $this->checkVarFromParam($param,'helperSln');
		  $module = $this->checkVarFromParam($param,'moduleSln');
	...
	public function drawConfig($module, $additionalOptions = array()) {
		if(!frameSln::_()->getModule($module))
			return false; 
		// check for xml file with params structure  
	   if(frameSln::_()->getModule($module)->isExternal())
		   $config_xml = frameSln::_()->getModule($module)->getModDir(). 'mod.xml';
	   else
		   $config_xml = SLN_MODULES_DIR.$module.DS.'mod.xml';

	   if (!file_exists($config_xml)) {
		   // if there is no configuration file for this $module
		   return __('There are no configuration options for this module', SLN_LANG_CODE);
	   }
	   $output = '';
		   ...
		   foreach ($params as $param) {
			   ...
			   foreach ($configOptions as $key=>$value){
				  ...
				  // if helper is needed to render the object
					  ...
					  $helper = frameSln::_()->getModule($hmodule)->getHelper();
					  if ($helper) {
				  ...
				  // filling the parameters to build html element
					 $htmlParams = array('value'=>$fieldValue,'optionsSln'=>$options);
					 if($method == 'checkbox') {
						 $htmlParams['value'] = 1;
				  ...
				  // output label and html element
					 ...
					 $output .= '</label><br />';
					 $output .= htmlSln::$method($name,$htmlParams).'<br />';
					 $output .= '</div>';
			   }
			   $i++;
			 $output .= '</div>';
