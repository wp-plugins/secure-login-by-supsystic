...
class frameSln {
    ...
    public function __construct() {
        $this->_res = toeCreateObjSln('response', array());
        
    }
    static public function getInstance() {
        static $instance;
        if(!$instance) {
            $instance = new frameSln();
        }
        return $instance;
    }
        ...
        if($activeModules) {
            foreach($activeModules as $m) {
                ...
                if(!empty($m['ex_plug_dir'])) {
                    $moduleLocationDir = utilsSln::getExtModDir( $m['ex_plug_dir'] );
                }
                if(is_dir($moduleLocationDir. $code)) {
                    $this->_allModules[$m['code']] = 1;
                    if((bool)$m['active']) {
                        importClassSln($code. strFirstUp(SLN_CODE), $moduleLocationDir. $code. DS. 'mod.php');
                        $moduleClass = toeGetClassNameSln($code);
    ...
    public function init() {
		...
		dispatcherSln::doAction('afterModulesInit');
		
		modInstallerSln::checkActivationMessages();
        ...
        register_activation_hook(  SLN_DIR. DS. SLN_MAIN_FILE, array('utilsSln', 'activatePlugin')  ); //See classes/install.php file
        register_uninstall_hook(SLN_DIR. DS. SLN_MAIN_FILE, array('utilsSln', 'deletePlugin'));
        
		add_action('init', array($this, 'connectLang'));
        //$operationTime = microtime(true) - $startTime;
    }
	public function connectLang() {
		load_plugin_textdomain(SLN_CODE, false, SLN_DIR. 'lang');
	}
    /**
    ...
    public function checkPermissions($code, $action) {
        if($this->havePermissions($code, $action))
            ...
            exit(_e('You have no permissions to view this page', SLN_LANG_CODE));
        }
    }
    /**
    ...
    public function havePermissions($code, $action) {
        ...
        if($mod) {
            ...
            if(!empty($permissions)) {  // Special permissions
                if(isset($permissions[SLN_METHODS])
                    && !empty($permissions[SLN_METHODS])
                ) {
                    foreach($permissions[SLN_METHODS] as $method => $permissions) {   // Make case-insensitive
                        $permissions[SLN_METHODS][strtolower($method)] = $permissions;
                    }
                    if(array_key_exists($action, $permissions[SLN_METHODS])) {        // Permission for this method exists
                        $currentUserPosition = frameSln::_()->getModule('user')->getCurrentUserPosition();
                        if((is_array($permissions[ SLN_METHODS ][ $action ]) && !in_array($currentUserPosition, $permissions[ SLN_METHODS ][ $action ]))
                            || (!is_array($permissions[ SLN_METHODS ][ $action ]) && $permissions[SLN_METHODS][$action] != $currentUserPosition)
                        ) {
                            $res = false;
                        }
                    }
                }
                if(isset($permissions[SLN_USERLEVELS])
                    && !empty($permissions[SLN_USERLEVELS])
                ) {
                    $currentUserPosition = frameSln::_()->getModule('user')->getCurrentUserPosition();
					// For multi-sites network admin role is undefined, let's do this here
					if(is_multisite() && is_admin() && is_super_admin()) {
						$currentUserPosition = SLN_ADMIN;
					}
                    foreach($permissions[SLN_USERLEVELS] as $userlevel => $methods) {
                        if(is_array($methods)) {
    ...
    protected function _extractTables($tablesDir = SLN_TABLES_DIR) {
    ...
    protected function _extractTable($tableName, $tablesDir = SLN_TABLES_DIR) {
        importClassSln('noClassNameHere', $tablesDir. $tableName. '.php');
        $this->_tables[$tableName] = tableSln::_($tableName);
    }
    /**
     ...
     * @return object table
     * @example frameSln::_()->getTable('products')->getAll()
     */
    ...
    public function addScript($handle, $src = '', $deps = array(), $ver = false, $in_footer = false, $vars = array()) {
		$src = empty($src) ? $src : uriSln::_($src);
    ...
    /**
     * Add all scripts from _scripts array to worslness
     */
    ...
    public function isTplEditor() {
        $tplEditor = reqSln::getVar('tplEditor');
	...
	public function isAdminPlugOptsPage() {
		$page = reqSln::getVar('page');
		if(is_admin() && strpos($page, frameSln::_()->getModule('adminmenu')->getMainSlug()) !== false)
			return true;
		return false;
	}
