<?php
abstract class moduleSln extends baseObjectSln {
	...
	public function __construct($d/*, $params = array()*/) {
		...
		if(isset($d['ex_plug_dir']) && !empty($d['ex_plug_dir'])) {
			...
			$this->setExternalDir( utilsSln::getExtModDir($d['ex_plug_dir']) );
			$this->setExternalPath( utilsSln::getExtModPath($d['ex_plug_dir']) );
		}
	}
	...
	public function getModDir() {
		if(empty($this->_externalDir)) {
			return SLN_MODULES_DIR. $this->getCode(). DS;
		} else {
	...
	public function getModPath() {
		if(empty($this->_externalPath)) {
			return SLN_MODULES_PATH. $this->getCode(). '/';
		} else {
	...
	protected function _createController() {
		...
		if(file_exists($this->getModDir(). 'controller.php')) {
			$className = '';
			if(importSln($this->getModDir(). 'controller.php')) {
				$className = toeGetClassNameSln($this->getCode(). 'Controller');
			}
			if(!empty($className)) {
	...
	protected function _createHelper() {
		...
		if (file_exists($this->getModDir().'helper.php')) {
			...
			importClassSln($helper, $this->getModDir(). 'helper.php');
	...
	/*public function __call($name, $arguments) {
		...
		errorsSln::push(__(array('Module', $this->_code, 'method', $name, 'undefined')), errorsSln::FATAL);
	}*/
