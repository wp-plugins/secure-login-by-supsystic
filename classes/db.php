...
class dbSln {
     ...
     * #__  Worslness prefix
     * ^__  Store plugin tables prefix (@see SLN_DB_PREF if config.php)
    ...
    static public function prepareQuery($query) {
        ...
        return str_replace(
                array('#__', '^__', '@__'), 
                array($wpdb->prefix, SLN_DB_PREF, $wpdb->prefix. SLN_DB_PREF),
                $query);
    }
    ...
    static public function timeToDate($timestamp = 0) {
        if($timestamp) {
            if(!is_numeric($timestamp))
                $timestamp = dateToTimestampSln($timestamp);
    ...
    static public function dateToTime($date) {
        ...
        if(strpos($date, SLN_DATE_DL)) return dateToTimestampSln($date);
        $arr = explode('-', $date);
        return dateToTimestampSln($arr[2]. SLN_DATE_DL. $arr[1]. SLN_DATE_DL. $arr[0]);
    }
