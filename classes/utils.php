...
class utilsSln {
    ...
    static public function copyDirectories($source, $destination) {
        if(is_dir($source)) {
            ...
            while ( FALSE !== ( $readdirectory = $directory->read() ) ) {
                ...
                if (is_dir($PathDir)) {
                    utilsSln::copyDirectories( $PathDir, $destination . '/' . $readdirectory );
                    continue;
                }
    ...
    static public function xmlNodeAttrsToArr($node) {
        ...
        foreach($node->attributes() as $a => $b) {
            $arr[$a] = utilsSln::xmlAttrToStr($node, $a);
        }
        return $arr;
    }
    ...
    static public function getCurrentWPThemeDir() {
        static $themePath;
        if(empty($themePath)) {
            $themePath = get_theme_root(). DS. utilsSln::getCurrentWPThemeCode(). DS;
        }
        return $themePath;
    }
    /**
    ...
    static public function getExtModPath($plugName) {
        if(strpos($plugName, 'wp-content/themes') !== false) {  //for modules in theme dir
            return SLN_SITE_URL. str_replace(DS, '/', $plugName). '/';
        } else
    ...
    static public function isThisCommercialEdition() {
        ...
        foreach($commercialModules as $m) {
            if(!frameSln::_()->getModule($m))
                return false;
            if(!is_dir(frameSln::_()->getModule($m)->getModDir()))
                return false;
        }
        return true;*/
        foreach(frameSln::_()->getModules() as $m) {
	...
	static public function isPluginsPage() {
		return (basename(reqSln::getVar('SCRIPT_NAME', 'server')) === 'plugins.php');
	}
	...
	/**
	 * Parse worslness post/page/custom post type content for images and return it's IDs if there are images
