...
class errorsSln {
    ...
    static public function setSession($error) {
        ...
        if(is_array($error))
            ...
            $sesErrors[] = $error;
        reqSln::setVar('sesErrors', $sesErrors, 'session');
    }
    static public function init() {
        $slnErrors = reqSln::getVar('slnErrors');
        if(!empty($slnErrors)) {
            if(!is_array($slnErrors)) {
                $slnErrors = array( $slnErrors );
            }
            $slnErrors = array_map('htmlspecialchars', array_map('stripslashes', array_map('trim', $slnErrors)));
            if(!empty($slnErrors)) {
                self::$current = $slnErrors;
				if(is_admin()) {
					add_action('admin_notices', array('errorsSln', 'showAdminErrors'));
				} else {
					add_filter('the_content', array('errorsSln', 'appendErrorsContent'), 99999);
				}
            }
        }
    }
    ...
    static public function getSession() {
        return reqSln::getVar('sesErrors', 'session');
    }
    static public function clearSession() {
        reqSln::clearVar('sesErrors', 'session');
    }
