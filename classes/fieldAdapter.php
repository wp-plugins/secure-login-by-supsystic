...
class fieldAdapterSln {
    ...
    static public function _($fieldOrValue, $method, $type) {
        if(method_exists('fieldAdapterSln', $method)) {
            switch($type) {
                case self::DB:
    ...
    static public function userFieldDestHtml($field) {
        $field->htmlParams['optionsSln'] = array();
    ...
    static public function userFieldDestToDB($value) {
        return utilsSln::jsonEncode($value);
    }
    static public function userFieldDestFromDB($value) {
        return utilsSln::jsonDecode($value);
    }
    static public function taxDataHtml($field) {
        ...
        $categories = frameSln::_()->getModule('products')->getCategories();
        $brands = frameSln::_()->getModule('products')->getBrands();
        $cOptions = array();
        $bOptions = array();
        if(!empty($categories)) {
        ...
        return '<div>'. __('Apply To', SLN_LANG_CODE). '
            <div id="tax_address">
                <b>'. __('Address', SLN_LANG_CODE). '</b><br />
                '. __('Destination', SLN_LANG_CODE). ':'. htmlSln::checkboxlist('params[dest]', array('optionsSln' => $listOfDest)). '<br />
                '. __('Country', SLN_LANG_CODE). ':'. htmlSln::countryList('params[country]', array('notSelected' => true, 'value' => $field->value['country'])). '<br />
            </div>
            <div id="tax_category">
                <b>'. __('Categories', SLN_LANG_CODE). '</b><br />
                '. (empty($cOptions) ? __('You have no categories', SLN_LANG_CODE) : htmlSln::checkboxlist('params[categories][]', array('optionsSln' => $cOptions))). '<br />
                    <b>'. __('Brands', SLN_LANG_CODE). '</b><br />
                '. (empty($bOptions) ? __('You have no brands', SLN_LANG_CODE) : htmlSln::checkboxlist('params[brands][]', array('optionsSln' => $bOptions))). '<br />
            </div>
            <div>'. __('Tax Rate', SLN_LANG_CODE).': '. htmlSln::text('params[rate]', array('value' => $field->value['rate'])).'</div>
            <div>'. __('Absolute', SLN_LANG_CODE).': '. htmlSln::checkbox('params[absolute]', array('checked' => $field->value['absolute'])).'</div>
        </div>';
    }
    ...
    static public function getCountries($notSelected = false) {
        ...
        if(empty($options[ $notSelected ])) {
            ...
            if(empty(self::$countries))
                self::$countries = self::getCachedCountries();
            if($notSelected) {
				$options[ $notSelected ][0] = is_bool($notSelected) ? __('Not selected', SLN_LANG_CODE) : __($notSelected);
			}
    ...
    static public function getExtraFieldOptions($field_id) {
        ...
        $options = frameSln::_()->getModule('optionsSln')->getHelper()->getOptions($field_id);
        if (!empty($options)) {
    ...
    static public function getFieldAttributes($params){
        ...
        if (!empty($params->attr)) {
            foreach ($params->attr as $key=>$value) {
                $output .= __($key).':<br />';
                $output .= htmlSln::text('params[attr]['.$key.']',array('value'=>$value)).'<br />';
            }
        } else {
                $output .= __('class', SLN_LANG_CODE).':<br />';
                $output .= htmlSln::text('params[attr][class]',array('value'=>'')).'<br />';
                $output .= __('id', SLN_LANG_CODE).':<br />';
                $output .= htmlSln::text('params[attr][id]',array('value'=>'')).'<br />';
        }
        return $output;
    }
    /**
    ...
    static function productFieldCategories($field){
        if(!empty($field->htmlParams['optionsSln']))
            return;
        ...
        if(!empty($categories)) {
            if(!is_array($field->value['categories']))
                    $field->value['categories'] = array();
            $field->htmlParams['optionsSln'][0] = in_array(0,$field->value['categories'])?__('Deselect All'):__('Select All', SLN_LANG_CODE);
            foreach($categories as $c) {
                $field->htmlParams['optionsSln'][$c->term_taxonomy_id] = $c->cat_name;
            }
        }*/
    }
	...
	static public function getCachedCountries($clearCache = false) {
		if(empty(self::$countries) || $clearCache)
			self::$countries = frameSln::_()->getTable('countries')->getAll('id, name, iso_code_2, iso_code_3');
	...
	static public function getCachedStates($clearCache = false) {
		if(empty(self::$states) || $clearCache)
			self::$states = frameSln::_()->getTable('states')
				->leftJoin( frameSln::_()->getTable('countries'), 'country_id' )
