...
abstract class controllerSln {
	...
	protected function _createModel($name = '') {
		if(empty($name)) $name = $this->getCode();
		$parentModule = frameSln::_()->getModule( $this->getCode() );
		$className = '';
		if(importSln($parentModule->getModDir(). 'models'. DS. $name. '.php')) {
			$className = toeGetClassNameSln($name. 'Model');
		}
	...
	protected function _createView($name = '') {
		...
		if(importSln($parentModule->getModDir(). 'views'. DS. $name. '.php')) {
			$className = toeGetClassNameSln($name. 'View');
		}
	 ...
	 * @example :
	 return array(
			S_METHODS => array(
				'save' => array(SLN_ADMIN),
				'remove' => array(SLN_ADMIN),
				'restore' => SLN_ADMIN,
			),
			S_USERLEVELS => array(
	 ...
	 * Can be used on of sub-array - SLN_METHODS or SLN_USERLEVELS
	 */
	public function getPermissions() {
		return array();
	}
	public function getModule() {
		return frameSln::_()->getModule( $this->getCode() );
	}
	...
	public function getListForTbl() {
		...
		$page = (int) reqSln::getVar('page');
		$rowsLimit = (int) reqSln::getVar('rows');
		$orderBy = reqSln::getVar('sidx');
		$sortOrder = reqSln::getVar('sord');

		// Our custom search
		$search = reqSln::getVar('search');
		$search = dispatcherSln::applyFilters($this->getCode(). 'BeforeSearchForTbl', $search);
		if($search && !empty($search) && is_array($search)) {
			foreach($search as $k => $v) {
				$v = trim($v);
				if($v == '') continue;	// Can not check for empty - because it can be 0
				if($k == 'text_like') {
		...
		// jqGrid search
		$isSearch = reqSln::getVar('_search');
		if($isSearch) {
			$searchField = trim(reqSln::getVar('searchField'));
			$searchString = trim(reqSln::getVar('searchString'));
		...
		if($limitStart < 0)
 		...
 		dispatcherSln::doAction($this->getCode(). 'BeforeGetListForTbl', $model);
	...
	public function removeGroup() {
		$res = new responseSln();
		if($this->getModel()->removeGroup(reqSln::getVar('listIds', 'post'))) {
