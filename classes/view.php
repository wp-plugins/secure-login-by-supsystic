<?php
abstract class viewSln extends baseObjectSln {
    /* 
     * @deprecated
     */
    protected $_tpl = SLN_DEFAULT;
    /*
	...
	public function getPath($tpl) {
		$path = '';
		$code = $this->_code;
		$parentModule = frameSln::_()->getModule( $this->_code );
		$plTemplate = frameSln::_()->getModule('options')->get('template');		// Current plugin template
		if(empty($plTemplate) || !frameSln::_()->getModule($plTemplate))
			$plTemplate = '';
		if(file_exists(utilsSln::getCurrentWPThemeDir(). 'sln'. DS. $code. DS. $tpl. '.php')) {
            $path = utilsSln::getCurrentWPThemeDir(). 'sln'. DS. $code. DS. $tpl. '.php';
        } elseif($plTemplate && file_exists(frameSln::_()->getModule($plTemplate)->getModDir(). 'templates'. DS. $code. DS. $tpl. '.php')) {
			$path = frameSln::_()->getModule($plTemplate)->getModDir(). 'templates'. DS. $code. DS. $tpl. '.php';
	...
	public function getModel($code = '') {
		return frameSln::_()->getModule( $this->_code )->getController()->getModel($code);
	}
