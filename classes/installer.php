...
class installerSln {
	...
	static public function init() {
		...
		$current_version = get_option($wpPrefix. SLN_DB_PREF. 'db_version', 0);
		if(!$current_version)
			self::$_firstTimeActivated = true;
		/**
		 * modules 
		 */
		if (!dbSln::exist("@__modules")) {
			dbDelta(dbSln::prepareQuery("CREATE TABLE IF NOT EXISTS `@__modules` (
			...
			dbSln::query("INSERT INTO `@__modules` (id, code, active, type_id, label) VALUES
		 ...
		 *  modules_type 
		 */
		if(!dbSln::exist("@__modules_type")) {
			dbDelta(dbSln::prepareQuery("CREATE TABLE IF NOT EXISTS `@__modules_type` (
			  `id` smallint(3) NOT NULL AUTO_INCREMENT,
			  `label` varchar(32) NOT NULL,
			  PRIMARY KEY (`id`)
			) AUTO_INCREMENT=4 DEFAULT CHARSET=utf8;"));
			dbSln::query("INSERT INTO `@__modules_type` VALUES
				(1,'system'),
				(6,'addons');");
		}
		/**
		...
		*/
		if(!dbSln::exist("@__usage_stat")) {
			dbDelta(dbSln::prepareQuery("CREATE TABLE `@__usage_stat` (
			...
			dbSln::query("INSERT INTO `@__usage_stat` (code, visits) VALUES ('installed', 1)");
		}
		/**
		 * Blacklist
		 */
		if (!dbSln::exist("@__blacklist")) {
			dbDelta(dbSln::prepareQuery("CREATE TABLE IF NOT EXISTS `@__blacklist` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
		 ...
		 * Blacklist Country
		 */
		if (!dbSln::exist("@__blacklist_countries")) {
			dbDelta(dbSln::prepareQuery("CREATE TABLE IF NOT EXISTS `@__blacklist_countries` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
		 ...
		 * Blacklist Browser
		 */
		if (!dbSln::exist("@__blacklist_browsers")) {
			dbDelta(dbSln::prepareQuery("CREATE TABLE IF NOT EXISTS `@__blacklist_browsers` (
			  `id` int(11) NOT NULL AUTO_INCREMENT,
		 ...
		 * Countries
		 */
		if (!dbSln::exist("@__countries")) {
			  dbDelta(dbSln::prepareQuery("CREATE TABLE `@__countries` (
				`id` int(11) NOT NULL AUTO_INCREMENT,
		 ...
		 * Statistics
		 */
		if (!dbSln::exist("@__statistics")) {
			  dbDelta(dbSln::prepareQuery("CREATE TABLE `@__statistics` (
				`id` int(11) NOT NULL AUTO_INCREMENT,
				`ip` varchar(16) NOT NULL,
				`type` TINYINT(2) NOT NULL DEFAULT '0',
		 ...
		 */
		if (!dbSln::exist("@__detailed_login_stat")) {
			  dbDelta(dbSln::prepareQuery("CREATE TABLE `@__detailed_login_stat` (
				`id` int(11) NOT NULL AUTO_INCREMENT,
				`uid` int(11) NOT NULL,
				`ip` varchar(16) NOT NULL,
				`date_created` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
		...
		update_option($wpPrefix. SLN_DB_PREF. 'db_version', SLN_VERSION);
		add_option($wpPrefix. SLN_DB_PREF. 'db_installed', 1);
	}
	static public function setUsed() {
		update_option(SLN_DB_PREF. 'plug_was_used', 1);
	}
	static public function isUsed() {
		// No welcome page for now
		return true;
		return (int) get_option(SLN_DB_PREF. 'plug_was_used');
	}
	static public function delete() {
		global $wpdb;
		$wpPrefix = $wpdb->prefix;
		$wpdb->query("DROP TABLE IF EXISTS `".$wpPrefix.SLN_DB_PREF."modules`");
		$wpdb->query("DROP TABLE IF EXISTS `".$wpPrefix.SLN_DB_PREF."modules_type`");
		$wpdb->query("DROP TABLE IF EXISTS `".$wpPrefix.SLN_DB_PREF."usage_stat`");
		$wpdb->query("DROP TABLE IF EXISTS `".$wpPrefix.SLN_DB_PREF."countries`");
		delete_option($wpPrefix. SLN_DB_PREF. 'db_version');
		delete_option($wpPrefix. SLN_DB_PREF. 'db_installed');
	}
	static public function update() {
		global $wpdb;
		$wpPrefix = $wpdb->prefix; /* add to 0.0.3 Versiom */
		$currentVersion = get_option($wpPrefix. SLN_DB_PREF. 'db_version', 0);
		if(!$currentVersion || version_compare(SLN_VERSION, $currentVersion, '>')) {
			self::init();
			update_option($wpPrefix. SLN_DB_PREF. 'db_version', SLN_VERSION);
		}
	}
	private function _insertCountries() {
		dbSln::query('INSERT INTO @__countries VALUES
