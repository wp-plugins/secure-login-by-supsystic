...
class langSln {
	...
	static public function extract($d = array('dir' => '', 'langSln' => '')) {
		$data = array();
		if(isset($d['dir']) && !empty($d['dir']))
			$langDirPath = $d['dir'];
		else if(isset($d['langSln']) && !empty($d['langSln']))
			$langDirPath = SLN_LANG_DIR. $d['langSln']. DS;
		else
			$langDirPath = SLN_LANG_DIR. SLN_WPLANG. DS;
