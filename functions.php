 ...
 */
if(!function_exists('dateToTimestampSln')) {
    function dateToTimestampSln($date) {
        if(empty($a))
            return false;
        $a = explode(SLN_DATE_DL, $date);
...
if(!function_exists('importSln')) {
    function importSln($path) {
        if(file_exists($path)) {
            require($path);
            return true;
        }
...
if(!function_exists('importClassSln')) {
    function importClassSln($class, $path = '') {
        if(!class_exists($class)) {
            if(!$path) {
				$classFile = $class;
				if(strpos(strtolower($classFile), SLN_CODE) !== false) {
					$classFile = preg_replace('/'. SLN_CODE. '/i', '', $classFile);
				}
                $path = SLN_CLASSES_DIR. $classFile. '.php';
			}
            return importSln($path);
            ...
            /*if(class_exists('ReflectionClass')) {   //ReflectionClass supported begining from php5
                ...
                if(strpos($classFile, SLN_DIR) === false) {   //Class is not in our plugin directory
                    ...
                    if(!empty($plugins)) {
                        for($i = 0; $i < count($plugins); $i++) {
                            if(strpos($plugins[$i], SLN_PLUG_NAME) !== false) {   //Let's remove our plugin from list of active plugins
                                unset($plugins[$i]);
                            }
                        }
 ...
 */
if(!function_exists('toeGetClassNameSln')) {
    function toeGetClassNameSln($class) {
        $className = '';
		if(class_exists($class. strFirstUp(SLN_CODE)))
			$className = $class. strFirstUp(SLN_CODE);
		else if(class_exists(SLN_CLASS_PREFIX. $class))
            $className = SLN_CLASS_PREFIX. $class;
		else
            $className = $class;
        return $className;
    } 
 ...
 */
if(!function_exists('toeCreateObjSln')) {
    function toeCreateObjSln($class, $params) {
        $className = toeGetClassNameSln($class);
        $obj = NULL;
 ...
 * @return array
 */
if(!function_exists('prepareParamsSln')) {
	function prepareParamsSln(&$d=array(), &$options = array()) {
		if (!empty($d['params'])) {
			...
			if (is_array($d['params'])) {
				$params = utilsSln::jsonEncode($d['params']);
		...
		if(isset($d['code'])) {
			if ($d['code'] == '') {
				$d['code'] = prepareFieldCodeSln($d['label']).'_'.rand(0, 9999999);
			}
		}
		return $d;
	}
}
if(!function_exists('prepareFieldCodeSln')) {
	function prepareFieldCodeSln($string) {
