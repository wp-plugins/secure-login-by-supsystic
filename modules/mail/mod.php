<?php
class mailSln extends moduleSln {
	...
	public function addOptions($opts) {
		$opts[ $this->getCode() ] = array(
			'label' => __('Mail', SLN_LANG_CODE),
			'opts' => array(
				'mail_function_work' => array('label' => __('Mail function tested and work', SLN_LANG_CODE), 'desc' => ''),
				'notify_email' => array('label' => __('Notify Email', SLN_LANG_CODE), 'desc' => __('Email address used for all email notifications from plugin', SLN_LANG_CODE), 'html' => 'text', 'def' => get_option('admin_email')),
			),
		);
		return $opts;
	}
