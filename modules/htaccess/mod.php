<?php
class htaccessSln extends moduleSln {
	private $_content = '';
	...
	public function init() {
		parent::init();
		if(frameSln::_()->isAdminPlugOptsPage()) {
	...
	public function putContent($content, $forAdmin = false) {
		...
		if(is_writable($path)) {
			...
			$this->pushError(sprintf(__('File %s is not writable, put there next content using FTP or other filemanager: <pre class="slnHtaccessPre">%s<br /></pre>', SLN_LANG_CODE), $path, htmlspecialchars($content)));
			return false;
		}
	}
	...
	public function showUnsavedHtaccessErrors() {
		if($this->checkUnsavedHtaccess()) {
			$htaccessPageLink = frameSln::_()->getModule('options')->getTabUrl( $this->getCode() );
			$html = '<div class="error"><p>'.
				sprintf(__('Your have unsaved changes in your .htaccess file. Please check <a href="%s">this page</a>.', SLN_LANG_CODE), $htaccessPageLink)
			.'</p></div>';
			echo $html;
		}
	}
	public function checkUnsavedHtaccess() {
		// REMOVE THIS  !!!!!
		//return true;
		$allOpts = frameSln::_()->getModule('options')->getAll();
		...
		$htpasswdContent = frameSln::_()->getModule('options')->get('htaccess_passwd_content');
		$htpasswdPath = frameSln::_()->getModule('secure_login')->getModel()->getHtpasswdFilePath();
		...
		foreach($allOpts as $catKey => $cData) {
			foreach($cData['opts'] as $optKey => $opt) {
				if($opt['value'] && isset($opt['htaccessChange']) && $opt['htaccessChange']) {
					...
					if($needChange) {
						$model = frameSln::_()->getModule( $catKey )->getModel();
