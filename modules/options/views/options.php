<?php
class optionsViewSln extends viewSln {
	private $_news = array();
	public function getNewFeatures() {
		$res = array();
		$readmePath = SLN_DIR. 'readme.txt';
		if(file_exists($readmePath)) {
			$readmeContent = @file_get_contents($readmePath);
			if(!empty($readmeContent)) {
				$matchedData = '';
				if(preg_match('/= '. SLN_VERSION. ' =(.+)=.+=/isU', $readmeContent, $matches)) {
					$matchedData = $matches[1];
				} elseif(preg_match('/= '. SLN_VERSION. ' =(.+)/is', $readmeContent, $matches)) {
					$matchedData = $matches[1];
				}
    ...
    public function getAdminPage() {
		...
		if(isset($tabs[ $activeTab ]) && isset($tabs[ $activeTab ]['callback'])) {
			frameSln::_()->getModule('supsystic_promo')->getModel()->saveUsageStat('tab.'. $activeTab);
			$content = call_user_func($tabs[ $activeTab ]['callback']);
		}
		frameSln::_()->addJSVar('adminOptionsSln', 'slnActiveTab', $activeTab);
	...
	public function getTabContent() {
		return frameSln::_()->getModule('supsystic_promo')->getView()->getOverviewTabContent();
		...
		frameSln::_()->addScript('jquery.knob', SLN_JS_PATH. 'jquery.knob.min.js');
		frameSln::_()->addScript('admin.mainoptions', $this->getModule()->getModPath(). 'js/admin.mainoptions.js');
		frameSln::_()->addJSVar('admin.mainoptions', 'slnOccupancy', $occupancy);
