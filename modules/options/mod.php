<?php
class optionsSln extends moduleSln {
	private $_tabs = array();
	private $_options = array();
	private $_optionsToCategoires = array();	// For faster search
	
	public function init() {
		dispatcherSln::addAction('afterModulesInit', array($this, 'initAllOptValues'));
	}
	...
	public function getAdminPage() {
		if(installerSln::isUsed()) {
			return $this->getView()->getAdminPage();
		} else {
			return frameSln::_()->getModule('supsystic_promo')->showWelcomePage();
		}
	}
	public function getTabs() {
		if(empty($this->_tabs)) {
			$this->_tabs = dispatcherSln::applyFilters('mainAdminTabs', array(
				'main_page' => array('label' => __('Overview', SLN_LANG_CODE), 'callback' => array($this, 'getTabContent'), 'wp_icon' => 'dashicons-shield-alt', 'sort_order' => 0),
			));
	...
	public function getActiveTab() {
		$reqTab = reqSln::getVar('tab');
		return empty($reqTab) ? 'main_page' : $reqTab;
	}
	public function getTabUrl($tab = '') {
		static $mainUrl;
		if(empty($mainUrl)) {
			$mainUrl = frameSln::_()->getModule('adminmenu')->getMainLink();
		}
		if(!empty($tab)) {
			switch($tab) {
				case 'htaccess':
					$tab = 'system#slnHtaccessBlock';
					break;
			}
		}
	...
	public function getAll() {
		if(empty($this->_options)) {
			$this->_options = dispatcherSln::applyFilters('optionsDefine', array(
				'general' => array(
					'label' => __('General', SLN_LANG_CODE),
					'opts' => array(
						'send_stats' => array('label' => __('Send usage statistics', SLN_LANG_CODE), 'desc' => '', 'def' => '1', 'html' => 'checkboxHiddenVal'),
					),
				),
			));
